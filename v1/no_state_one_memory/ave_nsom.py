...
def evolve(s_n, average_payoff, pl, ql, step_size, v):
    pl_t = pl[:]; ql_t = ql[:]
    pl_r = [0 for _ in range(4)]; ql_r = [0 for _ in range(4)]
    for s_i in range(s_n):
        ...
        # dp = (np.dot([1, 0], p_o) - np.dot([pl[s_i], 1 - pl[s_i]], p_o)) * pl[s_i]
        dp = (np.dot([1, 0], p_o)[0] - np.dot([0, 1], p_o)[0]) * pl_t[s_i] * (1 - pl_t[s_i])
        # dq = (np.dot([1, 0], q_o) - np.dot([ql[s_i], 1 - ql[s_i]], q_o)) * ql[s_i]
        dq = (np.dot([1, 0], q_o)[0] - np.dot([0, 1], q_o)[0]) * ql_t[s_i] * (1 - ql_t[s_i])
